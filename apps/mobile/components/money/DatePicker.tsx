...
function MonthName({ year, month }: { year: number; month: number }): React.ReactElement {
  return <Text className="text-lg font-semibold text-surface-100">{new Date(year, month, 1).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</Text>
}
  ...
  return <Modal visible={visible} transparent animationType="fade" onRequestClose={onCancel} statusBarTranslucent navigationBarTranslucent>
    <Pressable onPress={onCancel} className="flex-1 items-center justify-center bg-black/80 px-6">
      <Pressable onPress={(event) => event.stopPropagation()} className="w-full max-w-md rounded-3xl border border-surface-700 bg-surface-900 p-5">
        <Text className="text-base font-medium text-surface-300">Select day</Text>
        <Text className="mt-2 text-4xl font-bold text-surface-100">{formatIso(draft)}</Text>
        <View className="mt-5 flex-row items-center justify-between">
          <MonthName year={year} month={month} />
          <View className="flex-row gap-2">
            <Pressable onPress={() => step(-1)} hitSlop={10} className="h-11 w-11 items-center justify-center rounded-full bg-surface-800"><ChevronLeft color="#e6e6e8" size={22} /></Pressable>
            <Pressable onPress={() => step(1)} hitSlop={10} className="h-11 w-11 items-center justify-center rounded-full bg-surface-800"><ChevronRight color="#e6e6e8" size={22} /></Pressable>
          </View>
        </View>
        <View className="mt-4 flex-row">{WEEKDAYS.map((day, index) => <Text key={index} className="flex-1 text-center text-base font-medium text-surface-400">{day}</Text>)}</View>
        <View className="mt-1">{monthGrid(year, month).map((week, weekIndex) => <View key={weekIndex} className="flex-row">{week.map((day, dayIndex) => {
          ...
          return <Pressable key={dayIndex} onPress={() => setDraft(iso)} className="h-11 flex-1 items-center justify-center">
            <View className={`h-9 w-9 items-center justify-center rounded-full ${selected ? 'bg-accent-600' : iso === isoToday() ? 'border border-surface-600' : ''}`}>
              <Text className={selected ? 'text-base font-semibold text-white' : 'text-base text-surface-200'}>{day}</Text>
            </View>
          </Pressable>
        })}</View>)}</View>
        <View className="mt-4 flex-row justify-end gap-2">
          <Pressable onPress={onCancel} className="rounded-xl px-5 py-3.5"><Text className="text-base font-semibold text-surface-300">Cancel</Text></Pressable>
          <Pressable onPress={() => onConfirm(draft)} className="rounded-xl bg-accent-600 px-6 py-3.5"><Text className="text-base font-semibold text-white">OK</Text></Pressable>
        </View>
      </Pressable>
    </Pressable>
  </Modal>
...
function DayTile({ label, detail, active, icon, onPress }: { label: string; detail: string; active: boolean; icon: React.ReactNode; onPress: () => void }): React.ReactElement {
  return <Pressable onPress={onPress} className={`flex-1 items-center rounded-2xl border py-5 ${active ? 'border-accent-500 bg-accent-500/15' : 'border-surface-800 bg-surface-900'}`}>
    {icon}
    <Text className={`mt-3 text-xl font-semibold ${active ? 'text-accent-300' : 'text-surface-100'}`}>{label}</Text>
    <Text className="mt-1 text-base text-surface-400">{detail}</Text>
  </Pressable>
}
  ...
  return <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose} statusBarTranslucent navigationBarTranslucent>
    <Pressable onPress={onClose} className="flex-1 justify-end bg-black/70">
      <Pressable onPress={(event) => event.stopPropagation()} className="rounded-t-3xl border-t border-surface-700 bg-surface-950 px-5 pb-10 pt-5">
        <Text className="mb-5 text-center text-2xl font-bold text-surface-100">Date</Text>
        <Pressable onPress={() => setCalendarOpen(true)} className="items-center rounded-2xl border border-surface-800 bg-surface-900 py-6">
          <CalendarDays color="#e6e6e8" size={30} />
          <Text className="mt-3 text-xl font-semibold text-surface-100">Select day</Text>
        </Pressable>
        <View className="mt-3 flex-row gap-3">
          <DayTile label="Yesterday" detail={dayMonth(yesterday)} active={value === yesterday} icon={<Moon color="#e6e6e8" size={28} />} onPress={() => pick(yesterday)} />
          <DayTile label="Today" detail={dayMonth(today)} active={value === today} icon={<Sun color="#e6e6e8" size={28} />} onPress={() => pick(today)} />
        </View>
