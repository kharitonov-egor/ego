...
export const inputClass = 'min-h-11 rounded-lg border border-surface-700 bg-surface-900 px-3 py-2.5 text-[16px] text-surface-100'
...
export function Label({ text, children }: { text: string; children: React.ReactNode }): React.ReactElement {
  return <View className="mb-3.5"><Text className="mb-1.5 text-[14px] font-semibold uppercase tracking-wide text-surface-400">{text}</Text>{children}</View>
}
...
export function Chips<T extends string>({ values, value, labels, onChange }: { values: readonly T[]; value: T; labels?: Partial<Record<T, string>>; onChange: (value: T) => void }): React.ReactElement {
  return <View className="flex-row flex-wrap gap-2">{values.map((item) => <Pressable accessibilityRole="button" accessibilityState={{ selected: value === item }} key={item} onPress={() => onChange(item)} className={`min-h-11 justify-center rounded-lg border px-3 py-2 ${value === item ? 'border-accent-500 bg-accent-500/15' : 'border-surface-700 bg-surface-900'}`}><Text className={`text-[16px] ${value === item ? 'font-semibold text-accent-400' : 'text-surface-300'}`}>{labels?.[item] ?? item}</Text></Pressable>)}</View>
}
  ...
  return <>
    ...
    >{PERIOD_PRESETS.map((item) => {
      ...
      return <Pressable
        ...
        className={`min-h-11 justify-center rounded-full border px-3 py-2 ${active ? 'border-accent-500/50 bg-accent-500/20' : 'border-surface-800 bg-surface-900'}`}
      ><Text className={`text-[14px] ${active ? 'font-semibold text-accent-400' : 'text-surface-300'}`}>{text}</Text></Pressable>
    })}</ScrollView>
    <CustomPeriodSheet
      visible={picking}
      value={custom}
