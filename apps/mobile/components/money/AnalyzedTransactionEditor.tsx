...
export default function AnalyzedTransactionEditor({
  ...
  if (itemized && draft.receipt) {
    return <View>
      <View className="mb-4 flex-row gap-2">{(['expense', 'income'] as CategoryKind[]).map((value) => <Pressable key={value} onPress={() => changeKind(value)} className={`flex-1 rounded-xl border px-3 py-4 ${kind === value ? 'border-accent-500 bg-accent-500/15' : 'border-surface-800 bg-surface-900'}`}><Text className={`text-center text-lg font-semibold capitalize ${kind === value ? 'text-accent-300' : 'text-surface-300'}`}>{value}</Text></Pressable>)}</View>
  ...
  return <View>
    ...
    <View className="flex-row gap-3"><View className="flex-1"><Label text="Amount in USD"><TextInput value={amount} onChangeText={setAmount} keyboardType="decimal-pad" placeholder="0.00" placeholderTextColor="#707078" className={inputClass} /></Label></View><View className="flex-1"><Label text="Date"><Pressable onPress={() => setDatePicking(true)} className={`${inputClass} flex-row items-center justify-between`}><Text className="text-lg text-surface-100">{formatIso(date)}</Text><CalendarDays color="#b5b5bc" size={21} /></Pressable></Label></View></View>
    ...
    <Label text="Account"><View className="gap-2">{accounts.map((account) => <Pressable key={account.id} onPress={() => setAccountId(account.id)} className={`rounded-xl border px-3 py-3 ${accountId === account.id ? 'border-accent-500 bg-accent-500/15' : 'border-surface-800 bg-surface-900'}`}><Text className={accountId === account.id ? 'text-accent-300' : 'text-surface-300'} style={{ color: accountId === account.id ? '#93c5fd' : '#b5b5bc' }}>{account.name}</Text></Pressable>)}</View></Label>
    <Label text={`${kind === 'income' ? 'Income' : 'Expense'} category`}><View className="flex-row flex-wrap gap-2">{categories.map((category) => <Pressable key={category.id} onPress={() => setCategoryId(category.id)} className={`rounded-full border px-4 py-3 ${categoryId === category.id ? 'border-accent-500 bg-accent-500/15' : 'border-surface-800 bg-surface-900'}`}><Text className={`text-lg ${categoryId === category.id ? 'font-semibold text-accent-300' : 'text-surface-300'}`}>{category.name}</Text></Pressable>)}</View>{categories.length === 0 && <Text className="mt-2 text-base text-amber-400">Create an active {kind} category before saving.</Text>}</Label>
    <Label text="Notes"><TextInput value={notes} onChangeText={setNotes} multiline maxLength={360} placeholder="Visible details from the image" placeholderTextColor="#707078" className={`${inputClass} min-h-20`} /></Label>
    {draft.receipt && <Text className="mb-3 text-base leading-6 text-amber-400">Changing this itemized expense to income will save one transaction without its item list.</Text>}
