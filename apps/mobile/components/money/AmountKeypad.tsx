...
function Key({ label, onPress }: { label: string; onPress: () => void }): React.ReactElement {
  return <Pressable accessibilityRole="button" accessibilityLabel={label} onPress={onPress} className="h-10 flex-1 items-center justify-center rounded-xl border border-surface-700 bg-surface-900/70 active:bg-surface-800">
    <Text className="text-[19px] font-semibold text-surface-100">{label}</Text>
  </Pressable>
}
...
function SideKey({ children, onPress, onLongPress, className = '' }: { children: React.ReactNode; onPress: () => void; onLongPress?: () => void; className?: string }): React.ReactElement {
  return <Pressable onPress={onPress} onLongPress={onLongPress} className={`items-center justify-center rounded-xl ${className}`}>{children}</Pressable>
}
...
export function AmountKeypad({ onKey, onOpenDate, onConfirm, confirmDisabled, confirmColor, busy = false }: { onKey: (key: string) => void; onOpenDate?: () => void; onConfirm: () => void; confirmDisabled: boolean; confirmColor: string; busy?: boolean }): React.ReactElement {
  return <View className="flex-row gap-1.5 px-3">
    <View className="flex-1 gap-1.5">{ROWS.map((row, index) => <View key={index} className="flex-row gap-1.5">{row.map((key) => <Key key={key} label={key} onPress={() => onKey(key)} />)}</View>)}</View>
    <View className="w-[19%] gap-1.5">
