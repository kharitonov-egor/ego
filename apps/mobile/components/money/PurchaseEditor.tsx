...
export default function PurchaseEditor({
  ...
  return <View>
    <Label text="Merchant"><TextInput value={merchant} onChangeText={setMerchant} placeholder="Store name" placeholderTextColor="#909099" className={inputClass} /></Label>
    <Label text="Purchase date"><Pressable onPress={() => setDatePicking(true)} className={`${inputClass} flex-row items-center justify-between`}><Text className="text-[16px] text-surface-100">{formatIso(date)}</Text><CalendarDays color="#b5b5bc" size={15} /></Pressable></Label>
    ...
    ].map(([label, value, setter]) => <View key={label as string} className="w-[48%]"><Text className="mb-1 text-[14px] font-semibold uppercase tracking-wide text-surface-400">{label as string}</Text><TextInput value={value as string} onChangeText={setter as (value: string) => void} keyboardType="decimal-pad" className={inputClass} /></View>)}</View>
    <Text className="mt-1.5 text-[14px] leading-5 text-surface-400">Grand total becomes the account transaction amount.</Text>
    {warnings.map((warning) => <Text key={warning} className="mt-1.5 text-[14px] leading-5 text-amber-400">{warning}. Check the values before saving.</Text>)}
    <View className="mb-1.5 mt-4 flex-row items-center justify-between"><Text className="text-[16px] font-bold text-surface-100">Items</Text><Pressable onPress={addItem} className="flex-row items-center rounded-lg border border-surface-700 px-2.5 py-1.5"><Plus color="#b5b5bc" size={13} /><Text className="ml-1 text-[14px] font-semibold text-surface-300">Add item</Text></Pressable></View>
    <View className="gap-2">{items.map((item, index) => <View key={index} className="rounded-lg border border-surface-800 bg-surface-900/60 p-2.5">
      <View className="flex-row items-center"><TextInput value={item.name} onChangeText={(value) => changeItem(index, { name: value })} placeholder="Item name" placeholderTextColor="#909099" className={`${inputClass} flex-1`} /><Pressable onPress={() => setItems((current) => current.filter((_, itemIndex) => itemIndex !== index))} className="ml-2 p-2"><Trash2 color="#fb7185" size={14} /></Pressable></View>
      <View className="mt-2 flex-row gap-2"><View className="flex-1"><Text className="mb-1 text-[14px] font-semibold uppercase tracking-wide text-surface-400">Quantity</Text><TextInput value={String(item.quantity)} onChangeText={(value) => changeItem(index, { quantity: Number(value) || 0 })} keyboardType="decimal-pad" className={inputClass} /></View><View className="flex-1"><Text className="mb-1 text-[14px] font-semibold uppercase tracking-wide text-surface-400">Unit price</Text><TextInput value={item.unitPriceCents === null ? '' : dollars(item.unitPriceCents)} onChangeText={(value) => changeItem(index, { unitPriceCents: value === '' ? null : cents(value) })} keyboardType="decimal-pad" placeholder="Optional" placeholderTextColor="#909099" className={inputClass} /></View></View>
      <View className="mt-2 flex-row flex-wrap gap-2"><View className="w-[48%]"><Text className="mb-1 text-[14px] font-semibold uppercase tracking-wide text-surface-400">Gross</Text><TextInput value={dollars(item.grossPriceCents)} onChangeText={(value) => changeItem(index, { grossPriceCents: cents(value) })} keyboardType="decimal-pad" className={inputClass} /></View><View className="w-[48%]"><Text className="mb-1 text-[14px] font-semibold uppercase tracking-wide text-surface-400">Discount</Text><TextInput value={dollars(item.discountCents)} onChangeText={(value) => changeItem(index, { discountCents: cents(value) })} keyboardType="decimal-pad" className={inputClass} /></View><View className="w-[48%]"><Text className="mb-1 text-[14px] font-semibold uppercase tracking-wide text-surface-400">Line price</Text><TextInput value={dollars(item.lineTotalCents)} onChangeText={(value) => changeItem(index, { lineTotalCents: cents(value) })} keyboardType="decimal-pad" className={inputClass} /></View></View>
    </View>)}</View>
