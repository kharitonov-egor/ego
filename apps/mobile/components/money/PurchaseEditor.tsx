...
export default function PurchaseEditor({
  ...
  return <View>
    <Label text="Merchant"><TextInput value={merchant} onChangeText={setMerchant} placeholder="Store name" placeholderTextColor="#3c4b73" className={inputClass} /></Label>
    <Label text="Purchase date"><Pressable onPress={() => setDatePicking(true)} className={`${inputClass} flex-row items-center justify-between`}><Text className="text-sm text-surface-100">{formatIso(date)}</Text><CalendarDays color="#8a93ab" size={17} /></Pressable></Label>
    ...
    ].map(([label, value, setter]) => <View key={label as string} className="w-[48%]"><Text className="mb-1.5 text-xs text-surface-400">{label as string}</Text><TextInput value={value as string} onChangeText={setter as (value: string) => void} keyboardType="decimal-pad" className={inputClass} /></View>)}</View>
    <Text className="mt-2 text-xs text-surface-500">Grand total becomes the account transaction amount.</Text>
    {warnings.map((warning) => <Text key={warning} className="mt-2 text-xs text-amber-400">{warning}. Check the values before saving.</Text>)}
    <View className="mb-2 mt-6 flex-row items-center justify-between"><Text className="text-sm font-medium text-surface-200">Items</Text><Pressable onPress={addItem} className="flex-row items-center rounded-lg border border-surface-700 px-2.5 py-1.5"><Plus color="#8a93ab" size={14} /><Text className="ml-1 text-xs text-surface-400">Add item</Text></Pressable></View>
    <View className="gap-3">{items.map((item, index) => <View key={index} className="rounded-xl border border-surface-800 bg-surface-900/60 p-3">
      <View className="flex-row items-center"><TextInput value={item.name} onChangeText={(value) => changeItem(index, { name: value })} placeholder="Item name" placeholderTextColor="#3c4b73" className={`${inputClass} flex-1`} /><Pressable onPress={() => setItems((current) => current.filter((_, itemIndex) => itemIndex !== index))} className="ml-2 p-2"><Trash2 color="#fb7185" size={17} /></Pressable></View>
      <View className="mt-2 flex-row gap-2"><View className="flex-1"><Text className="mb-1 text-[11px] text-surface-500">Quantity</Text><TextInput value={String(item.quantity)} onChangeText={(value) => changeItem(index, { quantity: Number(value) || 0 })} keyboardType="decimal-pad" className={inputClass} /></View><View className="flex-1"><Text className="mb-1 text-[11px] text-surface-500">Unit price</Text><TextInput value={item.unitPriceCents === null ? '' : dollars(item.unitPriceCents)} onChangeText={(value) => changeItem(index, { unitPriceCents: value === '' ? null : cents(value) })} keyboardType="decimal-pad" placeholder="Optional" placeholderTextColor="#3c4b73" className={inputClass} /></View></View>
      <View className="mt-2 flex-row gap-2"><View className="flex-1"><Text className="mb-1 text-[11px] text-surface-500">Gross</Text><TextInput value={dollars(item.grossPriceCents)} onChangeText={(value) => changeItem(index, { grossPriceCents: cents(value) })} keyboardType="decimal-pad" className={inputClass} /></View><View className="flex-1"><Text className="mb-1 text-[11px] text-surface-500">Discount</Text><TextInput value={dollars(item.discountCents)} onChangeText={(value) => changeItem(index, { discountCents: cents(value) })} keyboardType="decimal-pad" className={inputClass} /></View><View className="flex-1"><Text className="mb-1 text-[11px] text-surface-500">Line price</Text><TextInput value={dollars(item.lineTotalCents)} onChangeText={(value) => changeItem(index, { lineTotalCents: cents(value) })} keyboardType="decimal-pad" className={inputClass} /></View></View>
    </View>)}</View>
