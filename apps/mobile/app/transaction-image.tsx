...
export default function TransactionImage(): React.ReactElement {
  ...
  if (draft && money.snapshot) return <ScrollView className="flex-1 bg-surface-950 px-5 pt-4" keyboardShouldPersistTaps="handled"><View className="mb-5 flex-row items-start justify-between"><View className="flex-1 pr-3"><Text className="text-2xl font-bold text-surface-100">Check the transaction</Text><Text className="mt-2 text-base leading-6 text-surface-400">Correct anything the model misread before saving.</Text></View><Pressable onPress={() => { setDraft(null); setError(null) }} className="flex-row items-center rounded-xl border border-surface-700 px-3 py-3"><RotateCcw color="#b5b5bc" size={20} /><Text className="ml-2 text-base font-semibold text-surface-300">Try another</Text></Pressable></View><AnalyzedTransactionEditor snapshot={money.snapshot} draft={draft} onSaved={(target) => router.replace(target === 'purchases' ? '/(money)/purchases' : '/(money)/transactions')} /><View className="h-12" /></ScrollView>

  return <View className="flex-1 items-center justify-center bg-surface-950 px-6">
    <View className="h-16 w-16 items-center justify-center rounded-2xl bg-accent-500/15"><ScanLine color="#91c4ff" size={30} /></View>
    <Text className="mt-5 text-xl font-semibold text-surface-100">Analyze a transaction</Text>
    <Text className="mt-3 text-center text-[17px] leading-7 text-surface-400">Use one check, receipt, invoice, card slip, or screenshot. Ego sends the image to OpenRouter once and does not save it.</Text>
    {analyzing ? <View className="mt-8 items-center"><ActivityIndicator color="#91c4ff" /><Text className="mt-3 text-lg font-medium text-surface-300">Reading image...</Text></View> : missing ? <View className="mt-8 w-full rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4"><Text className="text-base leading-6 text-amber-300">{missing}</Text><Pressable onPress={() => router.replace(missingRoute)} className="mt-3 self-start rounded-xl bg-accent-600 px-4 py-3"><Text className="text-lg font-semibold text-white">Fix it</Text></Pressable></View> : <View className="mt-8 w-full gap-3"><Pressable onPress={() => void camera()} className="flex-row items-center justify-center rounded-2xl bg-accent-600 px-5 py-4"><Camera color="#fff" size={23} /><Text className="ml-2 text-lg font-semibold text-white">Take photo</Text></Pressable><Pressable onPress={() => void library()} className="flex-row items-center justify-center rounded-2xl border border-surface-800 bg-surface-900 px-5 py-4"><ImageIcon color="#b5b5bc" size={23} /><Text className="ml-2 text-lg font-medium text-surface-200">Choose from photos</Text></Pressable><Pressable onPress={() => void paste()} className="flex-row items-center justify-center rounded-2xl border border-surface-800 bg-surface-900 px-5 py-4"><ClipboardPaste color="#b5b5bc" size={23} /><Text className="ml-2 text-lg font-medium text-surface-200">Paste image</Text></Pressable></View>}
    {error && <Text className="mt-5 text-center text-base leading-6 text-red-400">{error}</Text>}
  </View>
}
