...
import { ArrowRight, Pencil, Plus, ScanLine, Search, Trash2 } from 'lucide-react'
import type {
...
export default function TransactionsView({ snapshot, actions, openTransactionId, onOpenPurchase, onAnalyze }: { snapshot: MoneySnapshot; actions: MoneyActions; openTransactionId?: string | null; onOpenPurchase: (id: string) => void; onAnalyze: () => void }): React.ReactElement {
  ...
  useEffect(() => { if (openTransactionId) setSelectedId(openTransactionId) }, [openTransactionId])
  ...
  return <div className="flex h-full flex-col"><PageHeader title="Transactions" subtitle={`${snapshot.transactions.length} recorded`} action={<div className="flex gap-2"><button className={subtleButtonClass} disabled={!canCreate || actions.busy} onClick={onAnalyze}><ScanLine size={15} />Analyze image</button><button className={buttonClass} disabled={!canCreate || actions.busy} onClick={() => setEditing('new')}><Plus size={15} />Add transaction</button></div>} />
    ...
    <div className="flex min-h-0 flex-1"><div className="min-w-0 flex-1 overflow-y-auto p-5">{transactions.length === 0 ? <EmptyState title={snapshot.transactions.length ? 'No matching transactions' : 'Record your first transaction'} detail={snapshot.accounts.length ? 'Add income, an expense, or a transfer between accounts.' : 'Create an account before recording transactions.'} actionLabel="Add transaction" onAction={() => setEditing('new')} /> : <div className="space-y-4">{Array.from(groups.entries()).map(([date, items]) => {
      ...
      return <section key={date}><div className="mb-1 flex items-center justify-between px-2"><h2 className="text-xs font-medium uppercase tracking-wide text-surface-500">{displayDate(date)}</h2><span className={`text-xs ${net < 0 ? 'text-rose-400' : 'text-emerald-400'}`}>{formatMoney(net, true)}</span></div><div className={`${panelClass} divide-y divide-surface-800 overflow-hidden`}>{items.map((transaction) => {
        ...
        const purchase = snapshot.purchases.find((purchaseItem) => purchaseItem.transactionId === transaction.id)
        return <button key={transaction.id} onClick={() => purchase ? onOpenPurchase(purchase.id) : setSelectedId(transaction.id)} className={`flex w-full items-center gap-3 p-3 text-left hover:bg-surface-800/60 ${selectedId === transaction.id ? 'bg-surface-800/70' : ''}`}><div className="flex h-9 w-9 items-center justify-center rounded-full text-white" style={{ backgroundColor: category?.color ?? '#3c4b73' }}><MoneyIcon name={category?.icon ?? (transaction.kind === 'transfer' ? 'ArrowRight' : 'Tag')} size={16} /></div><div className="min-w-0 flex-1"><div className="truncate text-sm text-surface-100">{purchase?.merchant ?? transactionTitle(transaction, snapshot)}</div><div className="mt-0.5 flex items-center gap-1 text-xs text-surface-500"><span>{account?.name ?? 'Archived account'}</span>{transaction.kind === 'transfer' && <><ArrowRight size={10} /><span>{transactionTitle(transaction, snapshot)}</span></>}</div>{transaction.notes && !purchase && <div className="mt-0.5 truncate text-xs italic text-surface-500">{transaction.notes}</div>}</div><span className={`font-medium ${transaction.kind === 'income' ? 'text-emerald-400' : transaction.kind === 'expense' ? 'text-rose-400' : 'text-accent-400'}`}>{transaction.kind === 'income' ? '+' : transaction.kind === 'expense' ? '-' : ''}{formatMoney(transaction.amountCents)}</span></button>
      })}</div></section>})}</div>}</div>
